...
external _load_feature_file : string -> string -> t list = "load_feature_file"

let load_feature_file dialect fname =
  if Sys.file_exists fname then Base.List.rev (_load_feature_file dialect fname)
  else (
    print_endline ("Feature File " ^ fname ^ " does not exist");
    [])

let tags_exists tags tag = Base.List.exists tags ~f:(Tag.compare tag)
...
let filter_pickles tags pickles =
  match tags with
  | [], [] -> pickles
  | allowed, [] -> Base.List.rev_filter pickles ~f:(pickles_exists allowed)
  | [], disallowed -> filter_not_pickles disallowed pickles
  | allowed, disallowed ->
      let filtered_pickles =
        Base.List.rev_filter pickles ~f:(pickles_exists allowed)
      in
      Base.List.rev_filter filtered_pickles ~f:(fun p ->
