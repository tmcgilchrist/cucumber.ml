...
type 'a step = {
  ...
  stepdef :
    Re.Group.t option ->
    Step.arg option ->
    'a option * OutcomeManager.t ->
    ('a option * OutcomeManager.t) Lwt.t;
}
...
let match_stepdefs step_defs pickle_step =
  match
    Base.List.filter step_defs ~f:(match_pickle_step_to_stepdef pickle_step)
  with
  | [ matched_user_stepdef ] ->
      matched_user_stepdef.stepdef
        (Step.find_groups pickle_step matched_user_stepdef.regex)
        (Step.argument pickle_step)
  | [] ->
      fun _ ->
...
let construct_monadic_pickles cucc pickle =
  ...
  let steps_to_run =
    Base.List.rev
      (Base.List.fold pickle_steps ~init:[] ~f:(fun accum s ->
           match_stepdefs cucc.stepdefs s :: accum))
  in
  let before_hooks =
    Pickle.construct_hooks cucc.before_hooks pickle >>= fun _ ->
    Lwt.return (None, OutcomeManager.create None)
  in
  let steps = Base.List.fold steps_to_run ~init:before_hooks ~f:Lwt.bind in
  steps >>= fun final_state ->
  Pickle.construct_hooks cucc.after_hooks pickle >>= fun _ ->
  Lwt.return final_state

let execute cucc file_name tag_expr =
  let pickles =
    Pickle.load_feature_file (Dialect.string_of_dialect cucc.dialect) file_name
  in
  let tags =
    match tag_expr with
