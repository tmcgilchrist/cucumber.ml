...
let manip_state state outcome =
  match state with
  | Some x ->
     Lwt_io.printl ("In Manip_State with some state: " ^ (string_of_int x.foo)) >>= fun _ ->
     Lwt.return (Some { foo = (succ x.foo) }, Cucumber.OutcomeManager.add outcome Cucumber.Outcome.Pass)
  | None ->
     Lwt_io.printl ("In Manip_State with no state setting foo to 12") >>= fun _ ->
     Lwt.return (Some { foo = 12 }, Cucumber.OutcomeManager.add outcome Cucumber.Outcome.Pass)

let first_given_func group arg (foo_state_opt, outcome) =
  Lwt_io.printl "I am in the given: I have a simple background" >>= (fun _ -> manip_state foo_state_opt outcome)
...
let main : unit Lwt.t =
  ...
  let more_pickles = Cucumber.LibLwt.execute foo "test/test_lwt_2.feature" None in
  let* outcomes1 = Lwt.all pickles in
  let* () = Lwt_list.iter_p (fun (world, o) ->
      match world with
      | Some x ->
